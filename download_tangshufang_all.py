...
ROOT_FOLDER = "E:\\github\\downloadnoe"

# Get a list of all files in the folder
all_files = os.listdir(f"{ROOT_FOLDER}\\index")
...
for txt_file in txt_files:
    url_lists = []
    with open(f"{ROOT_FOLDER}\\index\\{txt_file}", "r") as f:
        url_lists = f.readlines()
    index = 1
    item_count = len(url_lists)

    # Get the base name and extension
    base_name, extension = os.path.splitext(os.path.basename(txt_file))

    target_folder = f"{ROOT_FOLDER}\\{base_name}"
    ...
    for url in url_lists:
        ...
        while no_change_counter < threshold or continue_scrolling:
            ...
            # Execute JavaScript to check if vertical scrollbar is present
            script = "return document.documentElement.scrollHeight > document.documentElement.clientHeight;"
            is_vertical_scrollbar_present = driver.execute_script(script)
            ...
            continue_scrolling = is_vertical_scrollbar_present and not is_body_bottom_displayed
        ...
        if item_count < 100:
            filename = target_folder + "\\{:02d}".format(index) + clean_filename(driver.title) + '.pdf'
        else:
            filename = target_folder + "\\{:03d}".format(index) + clean_filename(driver.title) + '.pdf'
        index += 1
        ...
        print(f"Saved:'{filename}', {index -1}/{item_count}")
